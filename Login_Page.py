...
def Sign_Up(root, window, Credentials, Visuals):
...
def ButtonCheck(Event, Button):
    ...
    print(button["username"]+button["password"]+button["email"])
...
def Registration(root, Credentials, Visuals):
    ...
    # Welcome Label
    ttk.Label(lFrame, text="Welcome To Lunar!", font=Visuals.Header, foreground="Black",
    ...
    ttk.Label(SmallFrame, text="Already have an account?", font=Visuals.Text, anchor="e").grid(column=1, row=3,
                                                                                            sticky="nes")
    ...
    # Sign Up Button
    Button = ttk.Button(rFrame, text="Sign Up", bootstyle="success", command=lambda: Sign_Up(root, TopLevel, Credentials, Visuals), state="disabled")
...
def Login(root, Credentials, Visuals):
    ...
    # Sign Up Button
    ttk.Button(rFrame, text="Sign In", bootstyle="success").grid(column=1, row=7, sticky="we", padx=100)
